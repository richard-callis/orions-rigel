    ...
    >
      <div className="flex items-center justify-between border-b border-border px-3 py-2">
        ...
        <button
          ...
          className="flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs hover:bg-black/5 dark:hover:bg-white/5 transition-colors disabled:opacity-40 cursor-pointer"
      ...
      <div className="border-b border-border">
        ...
        <div className="flex items-center justify-between bg-[#0d1117] px-3 py-2">
          <span className="text-xs text-white/30">⌘/Ctrl + Enter to run</span>
          <button
            onClick={() => execute(code)}
            disabled={status !== "ready" || running}
            className="flex items-center gap-1.5 rounded-md bg-accent px-3 py-1.5 text-xs font-medium text-white hover:opacity-90 transition-opacity disabled:opacity-40 cursor-pointer"
          >
      ...
      <div className="flex-1 overflow-auto p-3 text-sm">
        {initError && (
          <p className="text-red-600 font-mono text-xs whitespace-pre-wrap">{initError}</p>
        )}
        {runError && (
          <p className="text-red-600 font-mono text-xs whitespace-pre-wrap">{runError}</p>
        )}
        {message && !runError && <p className="text-foreground/70">{message}</p>}
        {result && !runError && (
          <div className="overflow-auto">
              ...
              <thead>
                <tr>
                  {result.fields.map((f) => (
                    <th
                      key={f.name}
                      className="sticky top-0 border-b border-border bg-surface px-2 py-1.5 text-left font-medium whitespace-nowrap"
                    >
                      {f.name}
                    </th>
                  ))}
              ...
              <tbody>
                {result.rows.map((row, i) => (
                  <tr key={i} className="odd:bg-black/[0.02] dark:odd:bg-white/[0.02]">
                    {result.fields.map((f) => (
                      <td key={f.name} className="border-b border-border px-2 py-1.5 whitespace-nowrap">
            ...
            {result.rows.length === 0 && (
              <p className="text-foreground/50 py-2">Query returned 0 rows.</p>
            )}
          </div>
        )}
      </div>
